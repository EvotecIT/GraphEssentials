...
- Include the Entra MDM enrollment app ID in full paged device inventory as MdmEnrollmentAppId for action audit reporting.

### 0.0.65

- Report page counts, fetched records, elapsed time, and retries for Entra, Intune, and Windows Autopilot inventories when progress is requested.
- Stop and return no inventory when a required cloud page remains unreadable after retries.
- Preserve date types and synchronized cache scope on the progress path.

### 0.0.64

- Read Windows Autopilot inventory without selected properties so affected Intune tenants can complete device inventory.

### 0.0.63

- Mark duplicate or conflicting Windows Autopilot associations as ambiguous instead of choosing an identity for device cleanup.
- Report page counts, fetched device counts, retries, and completion for large computer inventories when requested.
- Page and report progress for Intune computer inventory; retry an unreadable Graph response stream on its page.

### 0.0.62

- Page large device inventories for computer cleanup.
